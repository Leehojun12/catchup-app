...
function getApiKey() {
  return process.env.EXPO_PUBLIC_ODSAY_API_KEY?.trim() || '';
}

export function hasOdsayApiKey() {
  return !!getApiKey();
}

function shouldUseDevProxy() {
  if (process.env.EXPO_PUBLIC_ODSAY_USE_BACKEND === 'true') return true;
  if (process.env.EXPO_PUBLIC_ODSAY_USE_BACKEND === 'false') return false;
  return typeof __DEV__ !== 'undefined' && __DEV__;
}

async function fetchTransitViaBackend(origin, destination, walkFallbackPath) {
  const res = await api.getTransitRoute({
    origin,
    destination,
    walkPath: walkFallbackPath,
  });
  return res.transit || null;
}
...
async function searchTransitPaths(origin, destination, opt = 0) {
  ...
  if (data.error?.length) {
    const message = data.error[0]?.message || 'ODsay API 오류';
...
export async function buildTransitRouteMode(origin, destination, walkFallbackPath = null) {
  if (shouldUseDevProxy()) {
    try {
      const proxied = await fetchTransitViaBackend(origin, destination, walkFallbackPath);
      if (proxied) return proxied;
    } catch (err) {
      console.warn('ODsay dev proxy:', err.message);
    }
  }

  if (!hasOdsayApiKey()) return null;
